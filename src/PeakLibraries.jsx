...
export default function PeakLibraries({ month, records = [], overviewRows = [], warnings = [], onClose, onOpenLibrary }) {
  if (!month) return null
  const allowed = new Set(overviewRows.filter((row) => row.month === month).map((row) => [row.month, row.controlType, row.kingdom, row.pipeline, row.name].join("\u0000")))
  ...
  return <div className="peak-backdrop" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose() }}>
    <aside className="peak-drawer" role="dialog" aria-modal="true" aria-labelledby="peak-title">
      <div className="peak-head"><div><span className="kicker">READ VOLUME PEAK</span><h2 id="peak-title">{prettyDate(month)}</h2><p>{libraries.length} libraries · {total.toLocaleString()} matching reads</p></div><button onClick={onClose} aria-label="Close library breakdown"><X size={18} /></button></div>
      ...
      <div className="peak-list">{libraries.map((item) => <button key={item.libraryId} onClick={() => onOpenLibrary(item.libraryId)}>
        ...
        <ArrowRight size={15} />
      </button>)}{!libraries.length && <div className="explorer-empty compact"><AlertTriangle size={21} /><h2>No library records match</h2><p>Try clicking another month or widening the overview filters.</p></div>}</div>
    </aside>
  </div>
}
